...
bool_t cond_doit(cc_t cc, cond_t cond) 
{
    ...
    switch(cond)
    {
        ...
        default:
        {
            err_print("Wrong condition bit 0x%4x",cond);
        }
    }
    return doit;
}
...
stat_t nexti(y64sim_t *sim)
{
    ...
    /*sim->m is rip;sim->r is the memory to store regs' value*/
    ...
    icond = GET_FUN(codefun);
    next_pc++;

    /* get registers if needed (1 byte) */
    regid_t rA,rB;
    byte_t rAB=0xFF;
    /*Firstly, check the register byte is valid*/
    /*Cases are instructions using regs*/
    switch(icode)
    {
        case I_RRMOVQ:
        case I_IRMOVQ:
        ...
        case I_POPQ:
        {
            if(!get_byte_val(sim->m,next_pc,&rAB))
            {
                err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
                return STAT_ADR;
            }
    ...
    /*  Secondly, check the imm 8 bytes is valid
        Cases are instructions using imm
    ...
    switch (icode) {
      ...
      {
          if(icond > C_G || icond < C_YES)
          {
              err_print("PC = 0x%lx, Invalid instruction %.2x", sim->pc, codefun);
              return STAT_INS;
          }
          bool_t doit = cond_doit(sim->cc,icond);  
          if(doit)
          {
              set_reg_val(sim->r,rB,get_reg_val(sim->r,rA));
          }
          sim->pc = next_pc;
          break;
      }
      ...
      {
          imm += get_reg_val(sim->r,rB);
          if(!set_long_val(sim->m,imm,get_reg_val(sim->r,rA)))
          {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, imm);
            return STAT_ADR;
          }
          sim->pc = next_pc;
          break;
      ...
      case I_MRMOVQ: /* 5:0 regB:regA imm */
      {
          imm += get_reg_val(sim->r,rB);
          long_t value;
          if(!get_long_val(sim->m,imm,&value))
          {
              err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc,imm);
              return STAT_ADR;
          };
          set_reg_val(sim->r,rA,value);
          sim->pc = next_pc;
          break;
      }
      case I_ALU: /* 6:x regA:regB */
      {
          if(ifun < A_ADD || ifun >= A_NONE)
          {
              err_print("PC = 0x%lx, Invalid instruction %.2x", sim->pc,codefun);
              return STAT_ADR;
          }
          ...
          long_t valE = compute_alu(ifun,valB,valA);
      ...
      case I_CALL: /* 8:x imm */
      {
          ...
          valrsp -= 8;
          if(!set_long_val(sim->m,valrsp,next_pc))//Push ret addr.
          {
      ...
      {
          long_t valrsp=get_reg_val(sim->r,REG_RSP);
          if(!get_long_val(sim->m,valrsp,&next_pc))
          {
               err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, valrsp);
               return STAT_ADR;
          }
          set_reg_val(sim->r,REG_RSP,valrsp + 8);
          sim->pc = next_pc;
          break;
      }
      ...
      case I_POPQ: /* B:0 regA:F */
      {
          long_t valrsp = get_reg_val(sim->r,REG_RSP);
          long_t instack;
          if(!get_long_val(sim->m,valrsp,&instack))
          {
                err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, valrsp);
                return STAT_ADR;
          }
